...
object WondevSimulator {
  def nextLegalActions(state: WondevState): Seq[WondevAction] =
    nextLegalActions(MutableWondevState.fromSlowState(state))

  def next(
      fromState: MutableWondevState,
      wondevAction: WondevAction
  ): MutableWondevState = {
    ...
    wondevAction match {
      case MoveBuild(unitIndex, move, build) =>
        ...
        if (
          fromState.readable
            .isFree(build) || (build == fromState.readable.unitAt(unitIndex))
        ) {
  ...
  def nextLegalActions(state: MutableWondevState): Seq[WondevAction] = {
    ...
    val myUnits =
      if (state.nextPlayer) state.readable.myUnits else state.readable.opUnits
    ...
    while (id < myStart + 2) {
      ...
      while (nid < neighbors1.length) {
        ...
        if (
          WondevContext
            .isPlayable(h1) && h + 1 >= h1 && state.readable.isFree(target1)
        ) {
          ...
          while (nid2 < neighbors2.length) {
            ...
            if (
              WondevContext.isPlayable(h2) && (state.readable.isFree(
                target2
              ) || target2 == unit || myUnits.forall(_.distance(target2) > 1))
            ) {
      ...
      while (oid < opStart + 2) {
        ...
        if (WondevContext.isVisible(target1) && unit.distance(target1) == 1) {
          val pushTargets: Array[Pos] =
            WondevContext.pushTargets(state.size)(unit, target1)
          ...
          while (pid < pushTargets.length) {
            ...
            val h2 = state.readable.heightOf(target2)
            if (
              WondevContext.isPlayable(h2) && state.readable.heightOf(
                target1
              ) + 1 >= h2 && (myUnits.forall(
                _.distance(target2) > 1
              ) || state.readable.isFree(target2))
            ) {
