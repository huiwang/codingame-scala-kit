...
case class MctsAi[S <: GameState[Boolean], M](rules: RulesFor2p[S, M])
                                             (stopCondition: MctsNode[Boolean, S, M] => Boolean,
                                              randomPlay: S => Outcome[Boolean] = rules.randomPlay _) {

  def chooseMove(state: S): M = {
    @tailrec
    def iterate(node: MctsNode[Boolean, S, M]): M =
  ...
  def chooseMoveCount(state: S): (M, Int) = {
    @tailrec
    def iterate(node: MctsNode[Boolean, S, M], nodes: Int = 0): (M, Int) =
      if (stopCondition(node)) {
        (node.bestMove, nodes)
  ...
  def makeNode(state: S): MctsNode2p[S, M] = {
    new MctsNode2p(state, rules, randomPlay)
  }
