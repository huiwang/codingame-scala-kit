...
object StrikeBackGameReader {
  ...
  def readState(checkPoints: Vector[Disk])(): StrikeBackGameState = {
    val pods = CSBConstant.podIndices.map(i => {
      ...
      val angleVec = if (angle == -1) (checkPoints(nextcheckpointid).p - Vectorl(x, y)).norm else Vectorls.axisX.rotateInDegree(angle)
      (Disk(Vectorl(x, y), Vectorl(vx, vy), 400), angleVec, nextcheckpointid)
    })
