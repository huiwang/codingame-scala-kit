...
class AlphaBetaAiTest extends FlatSpec with Matchers {
  ...
  val dummyRules = new RulesFor2p[DummyState, String] {
    ...
    def validMoves(state: DummyState): Seq[String] =
      state.name match {
        case "a" => Seq("b", "f")
        case "b" => Seq("c", "d")
        case "f" => Seq("e", "g")
  ...
  def heuristic(state: DummyState): Double =
    state.name match {
      case "x" => 2
      case "z" => 4
      case _   => 0
  ...
  it should "select best move" in {
    val chosenMove =
      AlphaBetaAi(dummyRules, heuristic).chooseMove(dummyRules.initial, 3)
  ...
  case class IntsState(
      nextPlayer: Boolean = true,
      trueNumbers: List[Int] = Nil,
      falseNumbers: List[Int] = Nil
  ) extends GameState[Boolean] {

    def play(i: Int): IntsState =
      if (nextPlayer)
  ...
  it should "select move with higher heuristic and depth 1" in {
    val chosenMove = AlphaBetaAi(dummyUndecidedRules, heur)
      .chooseMove(dummyUndecidedRules.initial, 1)
    chosenMove shouldBe 10
  }

  it should "select move with higher heuristic and depth 2" in {
    val chosenMove = AlphaBetaAi(dummyUndecidedRules, heur)
      .chooseMove(dummyUndecidedRules.initial, 2)
    chosenMove shouldBe 10
  }

  it should "select move with higher heuristic and depth 3" in {
    val chosenMove = AlphaBetaAi(dummyUndecidedRules, heur)
      .chooseMove(dummyUndecidedRules.initial, 3)
    chosenMove shouldBe 10
  }
